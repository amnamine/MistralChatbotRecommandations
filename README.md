...
graph TD
    ...
    D -->|Queries RAG Examples| F[generated_sms.csv]
    D -->|API Calls| G[Mistral 7B LLM API]
```

### 1. AI Messaging Engine (`sms_generator.py`)
...
### 3. Java Gateway Orchestrator (`TelecomCampaignGateway.java`)
...
### 4. Interactive Web Interface (`index.html`, `style.css`, `app.js`)
...
- `cliensts_test2.csv`: Target customer segmentation parameters.
- `generated_sms (2).csv`: Master reference dataset containing past message examples.
- `sms_generator.py`: Module calling Mistral API with RAG context.
- `main.py`: FastAPI server exposing JSON endpoints.
- `TelecomCampaignGateway.java`: Java HTTP proxy and JSON logger.
- `campaign_runs.json`: Local transaction database for campaign runs.
- `index.html`, `style.css`, `app.js`: Premium dashboard web application.
- `run.bat`: Shell runner script to boot Python/Java servers and open the dashboard.
